...
/* ------------------------------------------------------------------
  ...
  YouTube search doesn't expose a live trending feed for arbitrary
  categories, so we simulate a genuinely-changing trending shelf by:
  1) Rotating the query pool based on the current hour+day (so the
     "trending" set actually changes through the day, not just once
     per page load).
  2) Adding freshness keywords ("this week", the current month/year)
     so results skew toward newer uploads.
  3) Auto-refetching on an interval so the shelf updates live while
     the app stays open, with a visible "updated Xs ago" indicator.
------------------------------------------------------------------- */
const TRENDING_POOL = [
...
// Picks a query deterministically from the pool based on the current
// hour-of-day, so refreshing later in the day surfaces a different
// "trending" slice — mirroring how real trending shelves rotate.
function getTrendingQueryForNow() {
  const now = new Date();
  const hourBucket = now.getHours() + now.getDate() * 24; // changes every hour, all day
  const base = TRENDING_POOL[hourBucket % TRENDING_POOL.length];
  const monthYear = now.toLocaleString("en-US", { month: "long", year: "numeric" });
  return `${base} ${monthYear}`;
}

const TRENDING_REFRESH_MS = 5 * 60 * 1000; // auto-refresh every 5 minutes
...
export default function App() {
  ...
  const fetchTrending = useCallback((isBackground = false) => {
    if (isBackground) setRefreshingTrending(true);
    else setLoadingPopular(true);
    const liveQuery = getTrendingQueryForNow();
    ytSearch(liveQuery, 20).then(({ error, results }) => {
  ...
  useEffect(() => {
    ...
    // Keep the trending shelf genuinely live: refetch on an interval,
    // and immediately refresh again when the tab regains focus/visibility
    // (covers the case where the hour rolled over while backgrounded).
    const interval = setInterval(() => fetchTrending(true), TRENDING_REFRESH_MS);
    const onVisible = () => {
          ...
          <div className="content-scroll">
            {view === "home" && (
              <HomeView popular={popular} loading={loadingPopular} error={popularError} playlists={playlists}
                ...
                trendingUpdatedAt={trendingUpdatedAt} refreshingTrending={refreshingTrending}
...
function HomeView({ popular, loading, error, playlists, onPlay, onOpenPlaylist, currentTrack, isPlaying, toggleLike, isLiked, onAddToPlaylist, trendingUpdatedAt, refreshingTrending, onRefreshTrending }) {
  ...
  useTicker(15000); // re-render every 15s so "Xs ago" stays fresh
    ...
    <div className="view-pad">
      ...
      <div className="section-header trending-header">
        ...
        <div className="trending-meta">
          ...
          <button
            className={`refresh-btn${refreshingTrending ? " spinning" : ""}`}
            onClick={onRefreshTrending}
            disabled={refreshingTrending}
            title="Refresh trending"
          >
            <Loader2 size={15} className={refreshingTrending ? "spin" : ""} />
          </button>
        </div>
      </div>
...
const STYLES = `
:root {
  --bg: #0A0C0E; --surface: #14181b; --surface-hi: #1D2226; --surface-hi-2: #262c31; --border: #2a3034;
  --text: #F5F7F4; --text-dim: #9BA6A2; --accent: #C4F135; --accent-2: #7B61FF; --radius: 14px;
  --radius-sm: 10px; --shadow-soft: 0 10px 30px rgba(0,0,0,.35); --shadow-lift: 0 16px 40px rgba(0,0,0,.45);
}
...
.app-shell ::-webkit-scrollbar-thumb { background: var(--surface-hi-2); border-radius: 8px; }
.app-shell ::-webkit-scrollbar-thumb:hover { background: var(--border); }
.ambient-glow { position: absolute; inset: 0; pointer-events: none; z-index: 0; transition: background 1.4s ease; }
.layout { flex: 1; display: flex; overflow: hidden; z-index: 1; padding: 10px 10px 0 10px; gap: 10px; min-width: 0; min-height: 0; }

.sidebar { width: 260px; flex-shrink: 0; background: linear-gradient(180deg, var(--surface) 0%, #101314 100%); border: 1px solid var(--border); border-radius: var(--radius); padding: 20px 14px; display: flex; flex-direction: column; overflow-y: auto; }
.brand { display: flex; align-items: center; gap: 10px; padding: 0 10px 24px; font-weight: 800; font-size: 19px; letter-spacing: -0.02em; }
.brand-mark { width: 32px; height: 32px; border-radius: 9px; background: linear-gradient(135deg, var(--accent), var(--accent-2)); color: #0B0E10; display: flex; align-items: center; justify-content: center; flex-shrink: 0; box-shadow: 0 4px 14px rgba(196,241,53,.25); }
.nav-group { display: flex; flex-direction: column; gap: 3px; }
.nav-item { display: flex; align-items: center; gap: 14px; background: none; border: none; color: var(--text-dim); font-size: 14.5px; font-weight: 700; padding: 10px 12px; border-radius: 9px; cursor: pointer; text-align: left; width: 100%; transition: color .15s, background .15s, transform .15s; }
.nav-item:hover { color: var(--text); background: var(--surface-hi); }
.nav-item.active { color: var(--text); background: var(--surface-hi-2); }
.mini-icon { width: 22px; height: 22px; border-radius: 6px; background: var(--surface-hi); display: flex; align-items: center; justify-content: center; color: var(--text-dim); flex-shrink: 0; }
.mini-icon.liked { background: linear-gradient(135deg, var(--accent-2), #4353c9); color: #fff; }
.count-pill { margin-left: auto; font-size: 11px; background: var(--surface-hi-2); padding: 2px 8px; border-radius: 10px; color: var(--text-dim); font-weight: 700; }
.sidebar-divider { height: 1px; background: var(--border); margin: 14px 4px; flex-shrink: 0; }
...
.playlist-row { display: flex; align-items: center; gap: 10px; background: none; border: none; padding: 7px 8px; border-radius: 9px; cursor: pointer; text-align: left; width: 100%; transition: background .15s; }
.playlist-row:hover, .playlist-row.active { background: var(--surface-hi); }
.playlist-row img, .playlist-row-placeholder { width: 40px; height: 40px; border-radius: 7px; object-fit: cover; flex-shrink: 0; background: var(--surface-hi); display: flex; align-items: center; justify-content: center; color: var(--text-dim); }
...
.main-pane { flex: 1; background: var(--surface); border: 1px solid var(--border); border-radius: var(--radius); overflow: hidden; display: flex; flex-direction: column; min-width: 0; }
.topbar { display: flex; align-items: center; gap: 14px; padding: 16px 26px; flex-shrink: 0; background: linear-gradient(180deg, rgba(255,255,255,.02), transparent); }
.topbar-nav { display: flex; gap: 8px; flex-shrink: 0; }
.round-btn { width: 32px; height: 32px; border-radius: 50%; background: rgba(0,0,0,.5); border: none; color: var(--text); display: flex; align-items: center; justify-content: center; cursor: pointer; flex-shrink: 0; transition: background .15s, transform .15s; }
.round-btn:hover:not(:disabled) { background: rgba(0,0,0,.7); transform: scale(1.05); }
.round-btn:disabled { opacity: .35; cursor: default; }
.settings-btn { width: 38px; height: 38px; border-radius: 50%; background: var(--surface-hi); border: 1px solid var(--border); color: var(--text-dim); display: flex; align-items: center; justify-content: center; cursor: pointer; flex-shrink: 0; transition: color .15s, border-color .15s, transform .15s; }
.settings-btn:hover { color: var(--text); border-color: var(--accent); transform: rotate(20deg); }
.search-box { flex: 1; max-width: 440px; display: flex; align-items: center; gap: 10px; background: var(--surface-hi); border: 1px solid var(--border); border-radius: 24px; padding: 10px 16px; min-width: 0; transition: border-color .15s, background .15s; }
.search-box:focus-within { border-color: var(--accent); background: var(--surface-hi-2); }
...
.clear-btn { background: var(--border); border: none; border-radius: 50%; width: 20px; height: 20px; display: flex; align-items: center; justify-content: center; color: var(--text); cursor: pointer; flex-shrink: 0; transition: transform .15s; }
...
.page-title { font-size: 27px; font-weight: 800; letter-spacing: -0.02em; margin: 10px 0 22px; }

.quick-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(200px, 1fr)); gap: 12px; margin-bottom: 36px; }
.quick-card { display: flex; align-items: center; gap: 12px; background: var(--surface-hi); border: 1px solid var(--border); border-radius: var(--radius-sm); overflow: hidden; cursor: pointer; position: relative; height: 62px; text-align: left; min-width: 0; transition: background .18s, transform .18s, border-color .18s; }
.quick-card:hover { background: var(--surface-hi-2); transform: translateY(-2px); border-color: #333b3f; }
.quick-card img, .quick-card-placeholder { width: 62px; height: 62px; object-fit: cover; flex-shrink: 0; background: var(--border); display: flex; align-items: center; justify-content: center; color: var(--text-dim); }
...
.section-header { display: flex; align-items: center; justify-content: space-between; margin: 32px 0 16px; flex-wrap: wrap; gap: 8px; }
.section-header h2 { font-size: 19px; font-weight: 800; letter-spacing: -0.01em; display: flex; align-items: center; gap: 10px; }
...
.trending-header { align-items: center; }
.live-badge { display: inline-flex; align-items: center; gap: 6px; font-size: 11px; font-weight: 800; letter-spacing: .04em; text-transform: uppercase; color: #FF6B6B; background: rgba(255,107,107,.12); padding: 4px 10px 4px 8px; border-radius: 20px; }
.live-dot { width: 7px; height: 7px; border-radius: 50%; background: #FF6B6B; box-shadow: 0 0 0 0 rgba(255,107,107,.6); animation: pulse-dot 1.8s infinite; }
@keyframes pulse-dot { 0% { box-shadow: 0 0 0 0 rgba(255,107,107,.55); } 70% { box-shadow: 0 0 0 7px rgba(255,107,107,0); } 100% { box-shadow: 0 0 0 0 rgba(255,107,107,0); } }
.trending-meta { display: flex; align-items: center; gap: 10px; }
.trending-updated { font-size: 12px; color: var(--text-dim); }
.refresh-btn { width: 28px; height: 28px; border-radius: 50%; background: var(--surface-hi); border: 1px solid var(--border); color: var(--text-dim); display: flex; align-items: center; justify-content: center; cursor: pointer; transition: color .15s, border-color .15s, transform .5s; flex-shrink: 0; }
...
.track-card { background: var(--surface-hi); border: 1px solid var(--border); border-radius: var(--radius-sm); padding: 12px; cursor: pointer; transition: background .18s, transform .18s, border-color .18s, box-shadow .18s; min-width: 0; }
.track-card:hover { background: var(--surface-hi-2); transform: translateY(-4px); border-color: #333b3f; box-shadow: var(--shadow-soft); }
.track-card-art-wrap { position: relative; margin-bottom: 10px; }
.track-card-art-wrap img { width: 100%; aspect-ratio: 1; object-fit: cover; border-radius: 8px; display: block; background: var(--border); }
.track-card-play { position: absolute; bottom: 8px; right: 8px; width: 38px; height: 38px; border-radius: 50%; background: var(--accent); display: flex; align-items: center; justify-content: center; opacity: 0; transform: translateY(6px) scale(.9); transition: all .2s; box-shadow: 0 8px 18px rgba(0,0,0,.5); }
.track-card:hover .track-card-play { opacity: 1; transform: translateY(0) scale(1); }
...
.track-row { display: grid; grid-template-columns: 28px 40px 1fr 30px 26px; align-items: center; gap: 10px; padding: 9px 10px; border-radius: 9px; cursor: pointer; transition: background .12s; min-width: 0; }
...
.genre-card { height: 96px; border: none; border-radius: var(--radius-sm); padding: 16px; font-weight: 800; font-size: 16px; color: #0B0E10; cursor: pointer; text-align: left; font-family: inherit; transition: transform .18s, filter .18s, box-shadow .18s; box-shadow: 0 6px 16px rgba(0,0,0,.2); }
.genre-card:hover { transform: translateY(-3px); filter: brightness(1.08); box-shadow: 0 14px 28px rgba(0,0,0,.32); }
...
.playlist-hero { display: flex; align-items: flex-end; gap: 20px; padding: 16px 0 10px; flex-wrap: wrap; }
.playlist-hero img, .playlist-hero-placeholder { width: 150px; height: 150px; object-fit: cover; border-radius: var(--radius-sm); box-shadow: var(--shadow-lift); background: var(--surface-hi); display: flex; align-items: center; justify-content: center; color: var(--text-dim); flex-shrink: 0; }
...
.playlist-hero-text h1 { font-size: 30px; font-weight: 900; letter-spacing: -0.03em; margin: 0; word-break: break-word; }
.pl-hero-sub { font-size: 13px; color: var(--text-dim); }
.playlist-actions { padding: 18px 0; display: flex; align-items: center; gap: 18px; }
.play-fab { width: 54px; height: 54px; border-radius: 50%; background: linear-gradient(135deg, var(--accent), #a8d92c); border: none; display: flex; align-items: center; justify-content: center; cursor: pointer; box-shadow: 0 10px 24px rgba(196,241,53,.3); transition: transform .15s; }
.play-fab:hover { transform: scale(1.07); }

/* Mini player */
.mini-player { flex-shrink: 0; background: var(--surface); border-top: 1px solid var(--border); z-index: 5; }
.mini-player.empty { display: flex; align-items: center; justify-content: center; height: 56px; margin: 8px; border-radius: var(--radius); border: 1px solid var(--border); }
.np-empty { color: var(--text-dim); font-size: 13px; }
.mini-player-row { display: grid; grid-template-columns: 1fr 2fr 1fr; align-items: center; padding: 0 18px; gap: 12px; height: 76px; }
.np-left { display: flex; align-items: center; gap: 12px; min-width: 0; cursor: pointer; }
.np-art { width: 50px; height: 50px; border-radius: 8px; object-fit: cover; flex-shrink: 0; background: var(--border); box-shadow: 0 4px 12px rgba(0,0,0,.3); }
...
.ctrl-btn { background: none; border: none; color: var(--text-dim); cursor: pointer; display: flex; align-items: center; justify-content: center; transition: color .15s, transform .15s; flex-shrink: 0; }
.ctrl-btn:hover { color: var(--text); transform: scale(1.08); }
.ctrl-btn.on { color: var(--accent); }
.play-btn { width: 36px; height: 36px; border-radius: 50%; background: #fff; border: none; display: flex; align-items: center; justify-content: center; cursor: pointer; transition: transform .12s; flex-shrink: 0; box-shadow: 0 4px 12px rgba(0,0,0,.25); }
...
.progress-knob { position: absolute; top: 50%; width: 11px; height: 11px; border-radius: 50%; transform: translate(-50%, -50%); opacity: 0; transition: opacity .15s; box-shadow: 0 2px 8px rgba(0,0,0,.4); }
